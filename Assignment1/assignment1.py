...
def process_numpy_arrays(
    array_list: list[np.ndarray], *, phred: bool = False
) -> tuple[np.ndarray, np.ndarray | None]:
    """Calculates the sum of a list of numpy arrays and counts occurrences per column.

    It handles differently sized arrays perfectly fine and is also able to apply phred
    score conversion (ascii-33) in a vectorized manner for all lines at once. By
    specifying the phred argument it will apply a phred score conversion and choose a
    fitting data type to reduce memory usage and improve computational performance.
    When the phred argument is True it will also calculate and return a count array,
    which helps track the amount of lines that have a character in a certain position.
    ...
    Returns:
        A tuple with two elements. The first element always being the numpy array with
        the sum/total for each position. The second element is None by default, but when
        phred is True a numpy array with the character count per position is returned.
    """
    use_dtype = np.uint8 if phred else np.int64
    ...
    # Create final 2-D array in the correct shape initially with zeros everywhere
    complete_array = np.zeros(bool_array.shape, dtype=use_dtype)

    # Concatenate the data of the lines into a 1-D array
    concatenated_array = np.concatenate(array_list, dtype=use_dtype)
    # If phred is True, perform conversion to phred scores (ASCII - 33)
    if phred:
        concatenated_array -= 33
    ...
    sum_array = np.sum(complete_array, axis=0, dtype=np.uint64)
    # Create the array with the count for each column if phred is True, otherwise None
    count_array = np.count_nonzero(bool_array, axis=0) if phred else None
...
class FastQChunk:
    ...
    Attributes:
        ...
        count_array: The amount of lines that have a character in a position.
    """
    def __init__(self, filepath: Path, start_offset: int, stop_offset: int):
        ...
        self.count_array: np.ndarray | None = None
...
class FastQFileHandler:
    ...
    def process_results(self, processed_chunks: list[FastQChunk]) -> None:
        ...
        Args:
            processed_chunks:
                A list of FastQChunk objects containing their sum- and count arrays.
        """
        ...
        for chunk in processed_chunks:
            chunk_results = [chunk.sum_array, chunk.count_array]
        ...
        for file, array_lists in chunks_per_file.items():
            # Unpack the sum and count arrays from the chunk results
            sum_arrays, count_arrays = zip(*array_lists)
    ...
    def show_results_for_file(
        ...
        Args:
            input_file_path: The path to the FastQ file the results are for.
            file_phred_averages: The average phred scores per position for the file.
        """
        if output_path := self.output_file:
            if len(self.file_paths) > 1:
                output_path = self.output_file.parent.joinpath(
                    f"{input_file_path.name}.{self.output_file.name}"
                )
        ...
        # If no output file was specified, print the results to stdout
        if len(self.file_paths) > 1:
